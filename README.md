<p align="center">
  <img src="assets/banner.svg" alt="Hot Path Heatmap" width="820">
</p>
...
- **Resource-aware scoring.** Database, HTTP/API, cache, message-queue, search, mail and filesystem
  operations anywhere down the call chain dominate the score, so a real DB call is never buried under
  trivial fan-out.
...
Works in any IntelliJ-based IDE (PhpStorm, WebStorm, IntelliJ IDEA, and the rest); it activates
whichever supported languages are present. Dynamic languages (Python, Ruby) are intentionally
excluded: their weak static call-resolution would make cost estimates misleading.
...
Each resource kind (DB / HTTP / cache / queue / search / mail / filesystem) weighs ~8 to 12 while
...
JVM languages, higher-order iteration callbacks (`forEach`/`map`/`reduce`/etc.) count as loops. Every
...
methods visited, a per-file time budget), so the editor never blocks on it. Built-in/library code is
